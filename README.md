...
1. **Web Scraping Rate Limits**: The Web Intelligence Module currently lacks robust proxy rotation. High-volume scraping of G2/Trustpilot may hit rate limits and fail gracefully.
2. **WebSocket Polling**: Real-time frontend updates are achieved through a 5-second polling interval. A true production implementation would use WebSockets via FastAPI for instant pushes.
3. **Draft Context Limitations**: Drafts generated by the agent rely solely on the RAG context and the conversation history. It does not look into external CRM ticket history outside of the current thread.
4. **Local ChromaDB Concurrency**: ChromaDB is running in persistent local mode inside the FastAPI process. In a scaled deployment, ChromaDB would need to run as a separate server.
...
```
                                                                  QUERY PLAN
-----------------------------------------------------------------------------------------------------------------------------------------------
 Sort  (cost=12.45..12.46 rows=4 width=128) (actual time=0.038..0.041 rows=4 loops=1)
   Sort Key: e."timestamp"
   Sort Method: quicksort  Memory: 25kB
   ->  Nested Loop  (cost=0.28..12.41 rows=4 width=128) (actual time=0.015..0.022 rows=4 loops=1)
         ->  Index Scan using idx_threads_sender_email on threads t  (cost=0.14..8.16 rows=1 width=4) (actual time=0.007..0.008 rows=1 loops=1)
               Index Cond: ((sender_email)::text = 'bob.jones@enterprise.net'::text)
         ->  Index Scan using idx_emails_thread_id on emails e  (cost=0.14..4.21 rows=4 width=128) (actual time=0.005..0.010 rows=4 loops=1)
...
```
                                                                  QUERY PLAN
-----------------------------------------------------------------------------------------------------------------------------------------------
 Sort  (cost=8.19..8.20 rows=2 width=16) (actual time=0.022..0.024 rows=3 loops=1)
   Sort Key: "timestamp"
   Sort Method: quicksort  Memory: 25kB
   ->  Index Scan using idx_emails_sender on emails  (cost=0.14..8.18 rows=2 width=16) (actual time=0.008..0.012 rows=3 loops=1)
         Index Cond: ((sender)::text = 'karen.w@retail-co.com'::text)
...
```
ChromaDB Query: "SLA breach credit obligation"
- Embedding generation (all-MiniLM-L6-v2): 35ms
- Vector similarity search (HNSW index): 4ms
Total retrieval latency: 39ms
```
